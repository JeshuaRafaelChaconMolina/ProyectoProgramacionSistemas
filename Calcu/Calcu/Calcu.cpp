...
// Cálculo de la media en ensamblador
double calcularMedia(const vector<int>& datos) {
    ...
    return static_cast<double>(suma) / n;  // Retornar la media
}

// Cálculo de la mediana en ensamblador
double calcularMediana(vector<int> datos) {
    int n = datos.size();
    __asm {
        ; Ordenar los datos en el vector(burbuja por ejemplo)
        xor ecx, ecx; ECX = 0 (índice)
        xor edx, edx; EDX = 0 (índice para comparar)
        sort_start:
        mov eax, datos[ecx * 4]; Cargar datos[ecx] en EAX
            cmp eax, datos[ecx * 4 + 4]; ; Comparar datos[ecx] con datos[ecx + 1]
            jg swap_elements; Si son desordenados, intercambiar
            inc ecx; ECX++
            cmp ecx, n - 1; Comparar si ECX < n - 1
            jl sort_start; Repetir el ciclo
            jmp done_sort
    ...
    return static_cast<double>(eax);  // Devolver la mediana
}

// Cálculo de la moda en ensamblador
...
// Cálculo del rango en ensamblador
...
// Cálculo de la desviación estándar en C++
